 ...
 * (SP) 800-38B.
 */
int32_t omac1_aes_vector(const uint8_t *key, size_t key_len, size_t num_elem,
             const uint8_t *addr[], const size_t *len, uint8_t *mac)
{
    ...
    while (left >= AES_BLOCK_SIZE) {
        ...
        if (left > AES_BLOCK_SIZE) aes_encrypt(ctx, cbc, cbc);
        left -= AES_BLOCK_SIZE;
    }
 ...
 * (SP) 800-38B.
 */
int32_t omac1_aes_128(const uint8_t *key, const uint8_t *data, size_t data_len, uint8_t *mac)
{
 ...
 * (SP) 800-38B.
 */
int32_t omac1_aes_256(const uint8_t *key, const uint8_t *data, size_t data_len, uint8_t *mac)
{
